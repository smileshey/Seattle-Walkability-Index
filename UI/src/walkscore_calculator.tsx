...
import { getSlopeScaler, getEffectiveSpeedLimitScaler, getBusinessDensityScaler, getCrimeDensityScaler, getCrashDensityScaler } from "./scaler_calculations";
  ...
  try {
    ...
    allFeatures.forEach((graphic) => {
      ...
      attributes.personalized_walkscore = baseWalkscore
        ? baseWalkscore * slopeScaler * effectiveSpeedLimitScaler * businessDensityScaler * crimeDensityScaler * crashDensityScaler
        : 0;
    });
    ...
    const colorVisVar = {
      ...
      stops: [
        { value: intervals[1] - 0.00001, color: new Color([230, 238, 207, 0.7]) },
        { value: intervals[2] - 0.00001, color: new Color([155, 196, 193, 0.7]) },
        { value: intervals[3] - 0.00001, color: new Color([105, 168, 183, 0.7]) },
        { value: intervals[4] - 0.00001, color: new Color([75, 126, 152, 0.7]) },
        { value: intervals[5], color: new Color([46, 85, 122, 0.7]) },
      ],
    };
...
const handleRecalculate = async (
  ...
  let personalizedNeighborhoodLayer = webMap.allLayers.find(
    (layer) => layer.title === "Personalized Neighborhood Walkscore"
  ) as FeatureLayer;
  ...
  if (personalizedFishnetLayer) {
    ...
    personalizedNeighborhoodLayer = webMap.allLayers.find(
      (layer) => layer.title === "Personalized Neighborhood Walkscore"
    ) as FeatureLayer;
